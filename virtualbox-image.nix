...
in {
  ...
  config = lib.mkIf cfg.enable {
    system.build.virtualBoxOVA = import (nixpkgs-path + /nixos/lib/make-disk-image.nix) {
      ...
      postVM =
        ''
          export HOME=$PWD
          export PATH=${pkgs.buildPackages.virtualbox}/bin:$PATH
